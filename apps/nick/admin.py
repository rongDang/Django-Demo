...
class Blog_Admin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        # 关于标签更新问题: obj.tag.all()不能获取到当前文章的标签，所以不能根据当前文章标签更新标签中的数目，
...
class CommentsAdmin(admin.ModelAdmin):
    list_display = ('id', 'entry', 'content', 'author', 'submit_date')
    list_display_links = ('id', 'entry')


@admin.register(CommentReply)
class ReplyAdmin(admin.ModelAdmin):
    list_display = ('id', 'comment', 'content', 'author_form', 'author_to', 'submit_date')
    list_display_links = ('id', 'comment')


@admin.register(Discuss)
class DiscussAdmin(admin.ModelAdmin):
    list_display = ('id', 'blog', 'content', 'parent_discuss', 'name', 'date')
